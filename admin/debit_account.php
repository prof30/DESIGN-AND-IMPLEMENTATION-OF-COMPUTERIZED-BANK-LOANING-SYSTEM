...
if(strlen($_SESSION['admin'])=="")
    ...
    if(isset($_POST['debit'])){
        ...
        if($_SESSION['old_amt'] < 1000 ){
            echo "<script> alert('Account Too Low for this Transaction') </script>";
            echo "<script>window.location = 'debit_account.php'</script>";
            exit();
        }else{
       ...
       if($success){
        ...
        $AmountPosted= '';
        $$DateofCredit = '';
        $Debited = mysqli_real_escape_string($con,$_POST['Amount']);
        $DateofDebit =  mysqli_real_escape_string($con, $_POST['Dateofdebit']);
       ...
       if($success){
           ...
           echo "<script> alert('Account Has been Debited') </script>";
...
<div class="form-group">
<label for="default" class="col-sm-2 control-label">Date of Debit</label>
<div class="col-sm-10">
<input type="Date" name="Dateofdebit" class="form-control" id="fullanme"  required="required" autocomplete="off">
</div>
</div>
  <div class="form-group">
   <div class="col-sm-offset-2 col-sm-10">
    <button type="submit" name="debit" class="btn btn-primary">Debit Account</button>
      </div>
     </div>
    </form>
